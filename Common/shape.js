...
function sphere( points_transform, max_subdivisions )		//max_subdivisions, 最好不要超过7，Build a sphere using subdivision, starting with a tetrahedron.  Store each level of detail in separate index lists.
	{	
		shape.call(this);	
...
function circle( num_tris, points_transform )		// Arrange triangles in a fan.  This version goes all the way around a circle with them.
	{	
		shape.call(this);	
		...
		this.createCircleVertices = function( recipient, num_tris ) 
			{	
				...
				{
						recipient.vertices.push( vec3( Math.cos(2 * Math.PI * counter/num_tris), Math.sin(2 * Math.PI * counter/num_tris), 0 ) );		
...
inherit(circle, shape);
	...
	circle.prototype.populate = function( recipient, num_tris, points_transform, center_idx )
		{
			if( center_idx === undefined )			// Not re-using a point?  Create one.
			{
				center_idx = recipient.vertices.push( vec3( mult_vec( points_transform, vec4( 0,0,0,1 ) ) ) ) - 1;
...
function axis()
	{
		...
		this.populate = ( function (self) 
			{	
				self.m_sphere = new sphere(); self.m_cube = new cube(); 
				self.m_cylinder = new cylindrical_strip(); self.m_fan = new triangle_fan_full;
				var stack = [];				
...
function cylinder(numRectangles, points_transform)
    {   if (points_transform == null)
	    points_transform = mat4();
	    shape.call(this);
		this.populate( numRectangles, points_transform);
		this.init_buffers();
	}
    inherit(cylinder, shape);

    cylinder.prototype.populate = function( recipient, numRectangles, points_transform)
	{   
		 //var m_cylinder = new cylindrical_strip();
		 //m_cylinder.prototype.populate(recipient,numRectangles, points_transform);
		//recipient.m_front = new circle(numRectangles, points_transform);
		//recipient.m_back = new circle(numRectangles, points_transform);
	}
	// cylinder.prototype.draw = function( model_transform, camera_transform, projection_transform, animation_time, texture_filename )
	// {   //shape.prototype.draw.call(this.model_transform, camera_transform, projection_transform, texture_filename);
	// 	this.m_cylinder.draw(model_transform, camera_transform, projection_transform);
	// 	// this.m_front.draw(model_transform, camera_transform, projection_transform, texture_filename);
	// 	// this.m_back.draw(model_transform, camera_transform, projection_transform, texture_filename);
...
function windmill( points_transform )
{
 ...
 var newNormal = mult_vec( transpose( inverse( rotation ) ), vec4( 0, 0, 1, 1 ) ); // rotation matrix for normals!
...
function fwing( points_transform )
{
 shape.call(this);
 if( !arguments.length) return; // Pass no arguments if you just want to statically call functions up the inheritance chain, and / or populate()
 this.populate( this, points_transform );
 ...
 fwing.prototype.populate = function( recipient, points_transform ) {
 var v =[];
 ...
 var i = 0;
 recipient.vertices.push( v[1]);
 recipient.vertices. push( v[3] );
 recipient.vertices. push( v[4] );
 ...
 recipient.texture_coords.push( vec2( 0, 1 ) );
 recipient.texture_coords.push( vec2( 1, 0 ) );
 recipient.indices.push ( 3*i ,3*i+1, 3*i+2); 

 i = 1;
 recipient.vertices.push( v[1]);
 recipient.vertices. push( v[4] );
 recipient.vertices. push( v[5] );
 recipient.normals. push( normal );
 recipient.normals. push( normal );
 recipient.normals. push( normal );
 recipient.texture_coords.push( vec2( 0, 0 ) );
 ...
 recipient.indices.push ( 3*i ,3*i+1, 3*i+2); 
  ...
  i = 3;
 recipient.vertices.push( v[3]);
 recipient.vertices. push( v[6] );
 recipient.vertices. push( v[7] );
 recipient.normals. push( normal );
 recipient.normals. push( normal );
 recipient.normals. push( normal );
 recipient.texture_coords.push( vec2( 0, 0 ) );
 ...
 recipient.indices.push ( 3*i ,3*i+1, 3*i+2);
  ...
  i = 21;
 recipient.vertices.push( v[2]);
 recipient.vertices. push( v[19] );
 recipient.vertices. push( v[21] );
 recipient.normals. push( normal );
 recipient.normals. push( normal );
 recipient.normals. push( normal );
 recipient.texture_coords.push( vec2( 0, 0 ) );
 ...
 function f_umbrella( points_transform )
{
 shape.call(this);
 if( !arguments.length) return; // Pass no arguments if you just want to statically call functions up the inheritance chain, and / or populate()
 this.populate( this, points_transform );
 ...
 inherit(f_umbrella, shape);

 f_umbrella.prototype.populate = function( recipient, points_transform ) {
 var v =[];
 ...
 l1 = subtract(v[5],v[1]);
 l2 = subtract(v[3],v[1]);
 //normal = cross(l1, l2)/(length(l1)*length(l2));
 normal = normalize(cross(l1, l2));
 recipient.normals. push( normal );
 recipient.normals. push( normal );
 recipient.normals. push( normal );
 ...
 function s_umbrella( points_transform )
{
 shape.call(this);
 if( !arguments.length) return; // Pass no arguments if you just want to statically call functions up the inheritance chain, and / or populate()
 this.populate( this, points_transform );
 ...
 inherit(s_umbrella, shape);

 s_umbrella.prototype.populate = function( recipient, points_transform ) {
 var v =[];
 var normal = [];
 var tmp1, tmp2, tmp3, tmp4, tmp5, l1, l2;
 v[1] = vec3 (0, 1, 0 ) ;
 v[2] = vec3 (4, 0, 0);
 v[3] = vec3 (0, 0, 4);
 v[4] = vec3 (0, 0, -4);
 ...
 l1 = subtract(v[3],v[1]);
 l2 = subtract(v[2],v[1]);
 normal[1] = normalize(cross(l1, l2));

 l1 = subtract(v[2],v[1]);
 l2 = subtract(v[4],v[1]);
 normal[2] = normalize(cross(l1, l2));
 
 l1 = subtract(v[4],v[1]);
 l2 = subtract(v[5],v[1]);
 normal[3] = normalize(cross(l1, l2));

 l1 = subtract(v[5],v[1]);
 l2 = subtract(v[3],v[1]);
 normal[4] = normalize(cross(l1, l2));
 

 tmp1 = normalize(add(normal[1] , normal[2]));
 tmp2 = normalize(add(normal[3] , normal[4]));
 tmp3 = normalize(add(normal[4] , normal[1]));
 tmp4 = normalize(add(normal[3] , normal[2]));
 tmp5 = normalize(add(tmp1, tmp2));

 var i = 0;
 recipient.vertices.push( v[1] );
 recipient.vertices. push( v[2] );
 recipient.vertices. push( v[3] );
 recipient.normals. push( tmp5);
 recipient.normals. push( tmp1);
 recipient.normals. push( tmp3 );
 recipient.texture_coords.push( vec2( 0, 0 ) );
 recipient.texture_coords.push( vec2( 0, 1 ) );
 recipient.texture_coords.push( vec2( 1, 0 ) );
 recipient.indices.push ( 3*i ,3*i+1, 3*i+2); 
...
//  var l1,l2,tmp1,tmp2,tmp3,tmp4,tmp5;
//  l1 = subtract(v[3],v[1]);
//  l2 = subtract(v[2],v[1]);
//  normal[1] = normalize(cross(l1, l2));
